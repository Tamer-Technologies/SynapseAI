import { collaborationSection } from "../constants";
...
const Collaboration = () => {
  ...
  const icons = [
    { id: "notion", element: <SiNotion className="w-full h-full" /> },
    { id: "darabricks", element: <SiDatabricks className="w-full h-full" /> },
    { id: "discord", element: <SiDiscord className="w-full h-full" /> },
    { id: "figma", element: <SiFigma className="w-full h-full" /> },
    { id: "react", element: <SiReact className="w-full h-full" /> },
    { id: "tailwindcss", element: <SiTailwindcss className="w-full h-full" /> },
  ];
  ...
  return (
    <Section
      className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:place-items-center"
      id="collaboration"
    >
      <div>
        <h2 className="mb-10">
          {collaborationSection.heading.part1}{" "}
          <br className="hidden sm:inline-block" />{" "}
          {collaborationSection.heading.part2}
        </h2>
        {collaborationSection.list.map((item) => {
          return (
            <div
              key={item.id}
              ...
              >
                ...
                <Arrow
                  className={`ml-auto h-4 w-4 transition-all duration-500 ${
                    item.id !== activeAccordion && "-rotate-90"
                  }`}
                  aria-label="arrow"
                />
              </button>
        ...
        <div className="mt-10 flex justify-center">
          <ButtonVariant
            className="font-sharp uppercase text-center px-8 py-3 font-semibold hover:text-primary-color1 duration-300"
            href={collaborationSection.cta.url}
          >
            {collaborationSection.cta.text}
          </ButtonVariant>
        </div>
      </div>
      <div className="flex flex-col gap-10">
        <p className="text-gray-md text-center lg:text-lg">
          {collaborationSection.subtitle}
          <br className="hidden sm:inline-block" />
          {collaborationSection.description}
        </p>
        <div>
